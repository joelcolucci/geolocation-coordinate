class Coordinate {
  constructor(coordinateDecimal, coordinateType) {
    ...
    this.minutes = ((this.degrees - Math.floor(this.degrees)) * 60);
  ...
  _toDecimalDegreesString() {
    return `${this.degrees.toFixed(3)}\u00B0 ${this.cardinalDirection}`;
  }

  _toDegreesDecimalMinutesString() {
    return `${this.degrees.toFixed(0)}\u00B0 ${this.minutes.toFixed(3)}' ${this.cardinalDirection}`;
  }

  _toDegreesMinutesSecondsString() {
    return `${this.degrees.toFixed(0)}\u00B0 ${this.minutes.toFixed(0)}' ${this.seconds.toFixed(0)}" ${this.cardinalDirection}`;
  }
}
